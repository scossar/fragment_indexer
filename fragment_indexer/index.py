...
def index_html(
    html_dir: Path, release: Path, registry: Path, max_tokens: int, collection_name: str
):
    ...
    collection = client.create_collection(
        collection_name, embedding_function=encoder.function
    )
    records = []
    audit = []
    for fragment, chunks in prepared:
        ...
        for ordinal, chunk in enumerate(chunks):
            ...
            metadata = {
                ...
                "section_heading": fragment.headings[-1]
                if fragment.headings
                else fragment.page.title,
            ...
            audit.append(
                {"id": chunk_id, "text": chunk.text, "body": chunk.body, **metadata}
            )
    ...
    for key, metadata, document in zip(
        stored["ids"], stored_metadata, stored_documents, strict=True
    ):
    ...
    if sql_ids != set(ids.values()) or not all(
        v["db_id"] in sql_ids for v in mapping.values()
    ):
    ...
    if records:
        query = collection.query(
            query_texts=[records[0][1]], n_results=min(3, len(records))
        )
        query_metadata = query["metadatas"]
        if (
            not query["ids"][0]
            or not query_metadata
            or not all(m["db_id"] in sql_ids for m in query_metadata[0])
        ):
    ...
    summary = {
        ...
        "versions": {
            name: version(name) for name in ("chromadb", "lxml", "tokenizers")
        },
        "pages": len(pages),
        "fragments": len(fragments),
        "chunks": len(records),
        "url_mappings": len(mapping),
        "max_observed_tokens": max((r[2]["token_count"] for r in records), default=0),
        "fragment_content_sha256": hashlib.sha256(
            "".join(
                f.identity + f.html_heading + f.html_fragment for f in fragments
            ).encode()
        ).hexdigest(),
    }
