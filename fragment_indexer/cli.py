...
def parser():
    root = argparse.ArgumentParser(description="Build local Hugo fragments and embeddings; never deploys")
    commands = root.add_subparsers(dest="command", required=True)
    build = commands.add_parser("build", help="Build and verify an isolated copy of a Hugo site")
    build.add_argument("--site", type=Path, required=True)
    build.add_argument("--output", type=Path, default=Path("output"))
    build.add_argument("--page-template", default="themes/zalgorithm_theme/layouts/page.html")
    ...
    query = commands.add_parser("query", help="Query a completed local snapshot and retrieve its HTML")
...
def build(args):
    ...
    with (output / ".build.lock").open("a") as lock:
        ...
        if not registry.exists() and (output / "current").is_symlink():
            raise ValueError("Permanent ID registry is missing; restore it before rebuilding")
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="fragment-hugo-") as temporary:
                ...
                subprocess.run([sys.executable, "-m", "fragment_indexer", "_index",
                                "--html", str(release / "minimal"), "--release", str(release),
                                "--registry", str(registry), "--max-tokens", str(args.max_tokens),
                                "--collection", args.collection], check=True)
                mapping_path = release / "data/fragments/sections.json"
                shutil.copy2(mapping_path, isolated / "data/fragments/sections.json")
                print("Building production HTML locally and verifying HTMX IDs", flush=True)
                ...
                summary["hugo_version"] = subprocess.check_output(["hugo", "version"], text=True).strip()
...
def query(args):
    ...
    encoder = MiniLM()
    if summary["embedding_model"] != encoder.name or summary["tokenizer_sha256"] != encoder.tokenizer_sha256:
        raise ValueError("Query encoder does not match the snapshot's recorded model/tokenizer")
    client = chromadb.PersistentClient(path=str(release / "chroma"), settings=Settings(anonymized_telemetry=False))
    collection = client.get_collection(summary["collection"], embedding_function=encoder.function)
    if args.limit < 1:
    ...
    with closing(sqlite3.connect(release / "sqlite/sections.db")) as db:
        for metadata, distance in zip(result["metadatas"][0], result["distances"][0]):
            ...
            if db_id in seen:
                continue
            seen.add(db_id)
            record = db.execute("SELECT html_heading, html_fragment FROM sections WHERE id=?", (db_id,)).fetchone()
            if record is None:
                raise ValueError(f"Missing fragment {db_id}")
            rows.append({"db_id": db_id, "url": metadata["fragment_url"], "distance": distance,
                         "html": "".join(record)})
...
def main():
    ...
    try:
        if args.command == "build":
            ...
            index_html(args.html, args.release, args.registry, args.max_tokens, args.collection)
